    ...
    while (!InFile.eof()) {                             //只要文件还没有结束，就继续循环
        ...
        switch (RowType) {
            ...
            case 'N' : {                                //如果是神经元的信息，则输入到神经元的容器中
                double GetBias;                         //获取偏置
                double GetActvtnFnctn;                  //获取激活函数
                InFile >> GetBias;                      //获取偏置
                InFile >> GetActvtnFnctn;               //获取激活函数
                MyNeuroVector.push_back(NeuroContainer(GetBias, GetActvtnFnctn));
                std::getline(InFile, ThisRow);          //将指针跳转到下一行开头
                break;
            }
            case 'L' : {
                unsigned int GetStartNeuro;
                unsigned int GetEndNeuro;
                InFile >> GetStartNeuro;                //获取起始神经元
                InFile >> GetEndNeuro;                  //获取结束神经元
                MyLayerVector.push_back(LayerContainer(GetStartNeuro, GetEndNeuro));
                std::getline(InFile, ThisRow);          //将指针跳转到下一行开头
                break;
            }
            case 'S' : {
                ...
                int GetLyingNeuro;
                double GetWeight;
                InFile >> GetCnnctNeuro;                //获取前端的神经元
                InFile >> GetLyingNeuro;                //获取后端的神经元
                InFile >> GetWeight;                    //获取权重
                auto result = MySynapseSet.insert(SynapseContainer(GetCnnctNeuro, GetLyingNeuro, GetWeight)); //将树突存到集合中
